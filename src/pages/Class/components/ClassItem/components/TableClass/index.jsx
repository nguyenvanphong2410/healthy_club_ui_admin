...
function TableClass({ handleChangeTableClass }) {
  ...
  const columns = [
    {
      title: <span className="title-table">Lớp thể thao</span>,
      dataIndex: 'pictures',
      key: 'pictures',
      width: 300,
      align: 'center',
    ...
    {
      title: <span className="title-table">Khóa thể thao</span>,
      dataIndex: 'course',
      key: 'course',
      width: 200,
    ...
    {
      title: <span className="title-table">Huấn luyện viên</span>,
      dataIndex: 'teacher',
      key: 'teacher',
      width: 200,
      showSorterTooltip: false,
    ...
    {
      title: <span className="title-table">Hội viên</span>,
      dataIndex: 'creator',
      key: 'creator',
      width: 150,
      showSorterTooltip: false,
      ...
      render: (text, record) => {
        return (
          <span className="text-blue-55 underline cursor-pointer" onClick={() => handleClickTotalStudent(record)}>
            {record.student_ids?.length > 0 ? record.student_ids?.length : 0} hội viên
          </span>
        );
      },
    },
    ...
    hasPermission([PERMISSIONS.EDIT.EDIT_CLASS, PERMISSIONS.DELETE.DELETE_CLASS])
      ? {
          title: <span className="title-table">Hành động</span>,
          dataIndex: 'actions',
          key: 'actions',
          align: 'center',
          ...
          render: (text, record) => {
            return (
              <div className={`btn-table-action`}>
                <Tooltip placement="top" title={'Xem thông tin lớp'}>
  ...
  return (
    <div>
      ...
      <ModalDefault
        ...
        title={<>Danh sách hội viên của lớp <span className='text-color-blue'>{classSelected?.name}</span> thuộc khóa thể thao <span className='text-color-main'>{classSelected?.course?.name}</span></>}
        width={1650}
      >
        <TableDSSVOfClass />
      </ModalDefault>
