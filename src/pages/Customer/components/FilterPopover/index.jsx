...
function FilterPopoverCustomer() {
  ...
  const handleSelect = (type, value) => {
    if (type === 'course_id_of_student') {
      handleFilterTableCustomer('course_id_of_student', value?.value)
    }
    if (type === 'class_id_of_student') {
      handleFilterTableCustomer('class_id_of_student', value?.value)
    }
  };
    ...
    <div className={`filter-wrap`}>
      <div className={`filter-content`}>
        <div className={`filter-title-wrap`}>
          <div className={`label-filter`}>Khóa thể thao:</div>
          <Select
            allowClear
            className={`main-select w-full`}
            placeholder="Chọn khóa thể thao"
            onChange={(type, value) => handleSelect('course_id_of_student', value)}
            options={optionCourse}
          />
        </div>
        <div className={`filter-title-wrap mt-3`}>
          <div className={`label-filter`}>Lớp thể thao:</div>
          <Select
            allowClear
            className={`main-select w-full`}
            placeholder="Chọn lớp thể thao"
            onChange={(type, value) => handleSelect('class_id_of_student', value)}
            options={optionClass}
          />
        </div>
...
export default FilterPopoverCustomer;
