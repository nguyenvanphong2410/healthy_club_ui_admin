...
const CourseAndClassOfTeacher = ({ dataCourseAndClassOfTeacher }) => {
  const { token } = theme.useToken();

  const getItems = (panelStyle) =>
    dataCourseAndClassOfTeacher?.map((item, index) => ({
      key: `${index + 1}`,
      label: (
        <p>
          Khóa thể thao: <span className="font-semibold">{item?.course?.name}</span>
        </p>
      ),
      children: (
  ...
  return (
    <>
    {
      dataCourseAndClassOfTeacher?.length > 0 ?
      <Collapse
        className="mt-4"
        bordered={false}
...
export default CourseAndClassOfTeacher;
