...
function* loadRouteData() {
  yield put(setTitlePage('Khóa thể thao'));
  yield put(
    setBreadcrumb([
      {
        path: '/',
...
function* handleActions() {
  yield takeLatest(createCourseSuccess, function* () {
    ...
    yield put(handleGetListDataCourses());
    getNotification('success', 'Tại mới khóa thể thao thành công');
  });
  ...
  yield takeLatest(createCourseFailure, function* (action) {
    ...
    if (errorStatus === 400) {
      ...
      getNotification('error', 'Tại mới khóa thể thao thất bại');
    }
  });
  ...
  yield takeLatest(updateCourseSuccess, function* () {
    yield put(setShowModalCreateOrUpdateCourse(false));
    yield put(handleGetListDataCourses());
    getNotification('success', 'Cập nhật khóa thể thao thành công');
  });
  ...
  yield takeLatest(updateCourseFailure, function* (action) {
    ...
    if (errorStatus === 400) {
      ...
      getNotification('error', 'Cập nhật khóa thể thao thất bại');
    }
  });
  ...
  yield takeLatest(deleteCourseSuccess, function* () {
    ...
    yield put(handleGetListDataCourses());
    getNotification('success', 'Xóa khóa thể thao thành công');
  });
  ...
  yield takeLatest(changeHighlightCourseSuccess, function* () {
    yield put(handleGetListDataCourses());
    getNotification('success', 'Cập nhật khóa thể thao phổ biến thành công');
  });
