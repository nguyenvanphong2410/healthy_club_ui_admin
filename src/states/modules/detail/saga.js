  ...
  yield put(
    setBreadcrumb([
      ...
      },
      {
        ...
        name: 'Hội viên ' + (name ? `${name} (${formatPhoneNumber(phone)})` : formatPhoneNumber(phone)),
      },
    ])
  );
...
function* handleActions() {
  ...
  yield takeLatest(requestChangeStatusOrderSuccess, function* () {
    ...
    const message = (order.statusOrderType === ORDER_STATUS['COMPLETED'] ? 'Xác nhận' : 'Hủy') + ' yêu cầu mua khóa thể thao thành công.'
