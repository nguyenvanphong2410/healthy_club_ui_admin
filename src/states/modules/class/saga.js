...
function* loadRouteData() {
  yield put(setTitlePage('Lớp thể thao'));
  yield put(
    setBreadcrumb([
      {
        path: '/',
  ...
  yield put(requestGetAllCustomers());
}
...
function* handleActions() {
  yield takeLatest(createClassSuccess, function* () {
    ...
    getNotification('success', 'Tạo mới lớp thể thao thành công.');
  });
  ...
  yield takeLatest(createClassFail, function* (action) {
    ...
    if (status === 400) {
      ...
      getNotification('error', 'Tạo mới lớp thể thao thất bại.');
    }
  });
  ...
  yield takeLatest(updateClassSuccess, function* () {
    ...
    yield put(setVisibleModalCreateOrUpdateClass(false));
    getNotification('success', 'Cập nhật lớp thể thao thành công.');
  });
  ...
  yield takeLatest(updateClassFail, function* (action) {
    ...
    if (status === 400) {
      ...
      getNotification('error', 'Cập nhật lớp thể thao thất bại.');
    }
  });
  ...
  yield takeLatest(deleteClassSuccess, function* () {
    ...
    getNotification('success', 'Xoá lớp thể thao thành công.');
    yield put(getListClasses());
    yield put(setShowModalDeleteClass(false));
  });

  yield takeLatest(deleteClassFail, function* () {
    yield call(getNotification, 'error', 'Xoá lớp thể thao thất bại.');
  });

  yield takeLatest(changeStatusClassSuccess, function* () {
    yield put(getListClasses());
    getNotification('success', 'Thay đổi trạng thái lớp thể thao thành công.');
  });

  yield takeLatest(changeStatusClassFail, function* () {
    yield put(getListClasses());
    getNotification('error', 'Thay đổi trạng thái lớp thể thao thất bại.');
  });
